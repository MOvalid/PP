...
public class SmallBallElf extends Elf{

    public SmallBallElf(String name, Elf nextElf, int boxSize){
        super(name, nextElf);
        if(boxSize <= 0) throw new IllegalArgumentException();
        else baubleBox = new SmallBall[boxSize];
    }
    public SmallBallElf(String name, Elf nextElf){
        this(name, nextElf, DEFAULT_BOX_SIZE);
    }

    public SmallBallElf(String name){
        this(name, null);
    }
    ...
    public void packTheBauble(Bauble bauble) {
        ...
        if (bauble instanceof SmallBall && numberOfBaubles < baubleBox.length){
    ...
    protected void fullBoxMessage(){
        ...
        System.out.print(" of Small Balls!\n\n");
    }
}
